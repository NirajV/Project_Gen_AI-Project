import { revertUpdateEvent, updateEvent } from './actions/event';

import {get, includes} from 'lodash';
import {addUndoItem} from './actions/undo-redo';
import {getUndoItem, getRedoItem} from './utils/selectors';

const createUndoMiddleware = ({getViewState, setViewState, revertingActions}) => {
  ...
  return ({dispatch, getState}) => (next) => (action) => {
    ...
    case 'UNDO_HISTORY@REDO': {
      ...
      if (redoItem) {
        ...
        dispatch(revertUpdateEvent(redoItem.action.payload.data, "Successfully completed the redo operation"));
        acting = false
      }
    }
      break
  ...
  revertingActions: {
    'UPDATE_EVENT_SUCCESS': {
      action: (action, { payload }) => {
        return revertUpdateEvent(payload, "Successfully completed the undo operation")
      },
      createArgs:  (state, action) => {
        return {
          payload: state.getIn(['event', 'data']).toJS()
        }
      }
    }
  }
